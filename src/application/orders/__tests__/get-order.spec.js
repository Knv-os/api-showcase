...
describe("GetOrder", () => {
    it("retorna o pedido pelo id", () => __awaiter(void 0, void 0, void 0, function* () {
        const repo = new InMemoryOrderRepo();
        const create = new CreateOrder_1.CreateOrder(repo);
        const get = new GetOrder_1.GetOrder(repo);
        const created = yield create.execute({
            clientId: (0, node_crypto_1.randomUUID)(),
            totalValue: 100,
        });
        const order = yield get.execute({ id: created.id });
        expect(order.id).toBe(created.id);
        expect(order.totalValue).toBe(100);
    }));
    it("lança erro quando id não existe", () => __awaiter(void 0, void 0, void 0, function* () {
        const repo = new InMemoryOrderRepo();
        const get = new GetOrder_1.GetOrder(repo);
        yield expect(get.execute({ id: (0, node_crypto_1.randomUUID)() })).rejects.toThrow("Order not found");
    }));
});
