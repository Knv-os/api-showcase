...
describe("UpdateOrder", () => {
    it("atualiza status e totalValue", () => __awaiter(void 0, void 0, void 0, function* () {
        const repo = new InMemoryOrderRepo();
        const create = new CreateOrder_1.CreateOrder(repo);
        const update = new UpdateOrder_1.UpdateOrder(repo);
        const created = yield create.execute({
            clientId: (0, node_crypto_1.randomUUID)(),
            totalValue: 100,
        });
        const updated = yield update.execute({
            id: created.id,
            status: "READY",
            totalValue: 150,
        });
    ...
    it("valida que ao menos um campo foi enviado", () => __awaiter(void 0, void 0, void 0, function* () {
        const repo = new InMemoryOrderRepo();
        const create = new CreateOrder_1.CreateOrder(repo);
        const update = new UpdateOrder_1.UpdateOrder(repo);
        const created = yield create.execute({
        ...
        yield expect(update.execute({ id: created.id })).rejects.toThrow("At least one field to update must be provided");
    }));
});
