...
describe("ListOrders", () => {
    it("lista pedidos criados", () => __awaiter(void 0, void 0, void 0, function* () {
        const repo = new InMemoryOrderRepo();
        const create = new CreateOrder_1.CreateOrder(repo);
        const list = new ListOrders_1.ListOrders(repo);
        yield create.execute({ clientId: (0, node_crypto_1.randomUUID)(), totalValue: 10 });
        yield create.execute({ clientId: (0, node_crypto_1.randomUUID)(), totalValue: 20 });
        const orders = yield list.execute();
        expect(orders).toHaveLength(2);
        expect(orders.map((o) => o.totalValue)).toEqual(expect.arrayContaining([10, 20]));
    }));
});
