...
describe("GetClient", () => {
    it("retorna cliente existente", () => __awaiter(void 0, void 0, void 0, function* () {
        const repo = new InMemoryClientRepo();
        const created = yield repo.create({ name: "ACME", phone: "1" });
        const usecase = new GetClient_1.GetClient(repo);
        const got = yield usecase.execute({ id: created.id });
        expect(got.id).toBe(created.id);
    }));
    it("erro quando cliente não encontrado", () => __awaiter(void 0, void 0, void 0, function* () {
        const repo = new InMemoryClientRepo();
        const usecase = new GetClient_1.GetClient(repo);
        yield expect(usecase.execute({ id: (0, node_crypto_1.randomUUID)() })).rejects.toThrow("Client not found");
    }));
});
